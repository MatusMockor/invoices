<x-app-layout>
    ...
    <form action="{{ route('invoices.update', $invoice) }}" method="POST" id="invoice-form">
            ...
            <div class="bg-white p-6 rounded-lg shadow">
                ...
                <div class="mb-4">
                    ...
                    <div class="mt-1 flex rounded-md shadow-sm">
                        <input type="text" name="ico" id="ico" value="{{ old('ico', $invoice->company->ico) }}" class="focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-md sm:text-sm border-gray-300" placeholder="Zadajte IČO">
                ...
                <div id="company-details" class="space-y-4">
                    <div>
                        ...
                        <input type="text" id="company_name" name="company_name" value="{{ old('company_name', $invoice->company->name) }}" readonly class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm bg-gray-50">
                    </div>
                    
                    <div>
                        <label for="company_address" class="block text-sm font-medium text-gray-700">Adresa</label>
                        <input type="text" id="company_address" name="company_address" value="{{ old('company_address', $invoice->company->street) }}" readonly class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm bg-gray-50">
                    </div>
                    
                    <div>
                        <label for="company_city" class="block text-sm font-medium text-gray-700">Mesto</label>
                        <input type="text" id="company_city" name="company_city" value="{{ old('company_city', $invoice->company->city) }}" readonly class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm bg-gray-50">
                    </div>
                    
                    <div>
                        <label for="company_postal_code" class="block text-sm font-medium text-gray-700">PSČ</label>
                        <input type="text" id="company_postal_code" name="company_postal_code" value="{{ old('company_postal_code', $invoice->company->postal_code) }}" readonly class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm bg-gray-50">
                    </div>
                    
                    <div>
                        <label for="company_dic" class="block text-sm font-medium text-gray-700">DIČ</label>
                        <input type="text" id="company_dic" name="company_dic" value="{{ old('company_dic', $invoice->company->dic) }}" readonly class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm bg-gray-50">
                    </div>
                    
                    <div>
                        <label for="company_ic_dph" class="block text-sm font-medium text-gray-700">IČ DPH</label>
                        <input type="text" id="company_ic_dph" name="company_ic_dph" value="{{ old('company_ic_dph', $invoice->company->ic_dph) }}" readonly class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm bg-gray-50">
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-lg shadow">
                ...
                <div class="grid grid-cols-1 gap-4">
                    <div>
                        ...
                        <input type="text" name="invoice_number" id="invoice_number" value="{{ old('invoice_number', $invoice->invoice_number) }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    </div>
                    
                    <div>
                        <label for="issue_date" class="block text-sm font-medium text-gray-700">Dátum vystavenia</label>
                        <input type="date" name="issue_date" id="issue_date" value="{{ old('issue_date', $invoice->issue_date->format('Y-m-d')) }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    </div>
                    
                    <div>
                        <label for="due_date" class="block text-sm font-medium text-gray-700">Dátum splatnosti</label>
                        <input type="date" name="due_date" id="due_date" value="{{ old('due_date', $invoice->due_date->format('Y-m-d')) }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" id="status" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            ...
                            <option value="sent" {{ old('status', $invoice->status) == 'sent' ? 'selected' : '' }}>Odoslaná</option>
                            <option value="paid" {{ old('status', $invoice->status) == 'paid' ? 'selected' : '' }}>Zaplatená</option>
                            <option value="cancelled" {{ old('status', $invoice->status) == 'cancelled' ? 'selected' : '' }}>Zrušená</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="currency" id="currency" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            ...
                            <option value="CZK" {{ old('currency', $invoice->currency) == 'CZK' ? 'selected' : '' }}>CZK</option>
                        </select>
                    </div>
                    
                    <div>
                        <label for="note" class="block text-sm font-medium text-gray-700">Poznámka</label>
                        <textarea name="note" id="note" rows="3" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('note', $invoice->note) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-lg shadow mb-6">
            ...
            <div class="flex flex-col">
                                    ...
                                    @foreach($invoice->items as $index => $item)
                                        <tr class="item-row">
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <input type="text" name="items[{{ $index }}][description]" value="{{ old('items.'.$index.'.description', $item->description) }}" class="item-description border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm" placeholder="Zadajte popis položky" required>
                                                <input type="hidden" name="items[{{ $index }}][id]" value="{{ $item->id }}">
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <input type="number" name="items[{{ $index }}][quantity]" value="{{ old('items.'.$index.'.quantity', $item->quantity) }}" class="item-quantity border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm w-20" min="1" required>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <input type="number" name="items[{{ $index }}][unit_price]" value="{{ old('items.'.$index.'.unit_price', $item->unit_price) }}" class="item-price border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm w-32" min="0" step="0.01" placeholder="0.00" required>
                                            </td>
                ...
                <div class="mt-6 text-right">
                    <div class="text-sm text-gray-500">Medzisúčet: <span id="subtotal">{{ number_format($invoice->total_amount / 1.2, 2, '.', '') }}</span> <span id="currency-display">{{ $invoice->currency }}</span></div>
                    <div class="text-sm text-gray-500">DPH (20%): <span id="vat">{{ number_format($invoice->total_amount - ($invoice->total_amount / 1.2), 2, '.', '') }}</span> <span id="currency-display2">{{ $invoice->currency }}</span></div>
                    <div class="text-lg font-bold">Spolu: <span id="grand-total">{{ number_format($invoice->total_amount, 2, '.', '') }}</span> <span id="currency-display3">{{ $invoice->currency }}</span></div>
        ...
        <div class="flex justify-end">
            <a href="{{ route('invoices.show', $invoice) }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150 mr-2">
                Zrušiť
            </a>
                ...
                if (table.querySelectorAll('.item-row').length > 1) {
                    ...
                    rows.forEach((row, index) => {
                        row.querySelectorAll('input').forEach(input => {
                            ...
                            if (name && !name.includes('[id]')) { // Skip the hidden ID field
