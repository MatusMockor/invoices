...
<body class="bg-gray-50 dark:bg-gray-900">
                ...
                <form class="space-y-4 md:space-y-6" method="POST" action="{{ request()->routeIs('oauth.login') ? route('oauth.login.store') : route('login') }}">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="text-sm font-medium text-indigo-600 hover:underline dark:text-indigo-500">{{ __('Forgot your password?') }}</a>
                        @endif
                    </div>

                    <button type="submit" class="w-full text-white bg-indigo-700 hover:bg-indigo-800 focus:ring-4 focus:outline-none focus:ring-indigo-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:focus:ring-indigo-800">{{ __('Sign in') }}</button>

                    <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                        {{ __("Don't have an account yet?") }} <a href="{{ route('register') }}" class="font-medium text-indigo-600 hover:underline dark:text-indigo-500">{{ __('Sign up') }}</a>
                    </p>
                </form>
            </div>
        </div>
    ...
    <!-- Theme toggle button -->
    <button id="theme-toggle" type="button" class="fixed bottom-4 right-4 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 rounded-lg text-sm p-2.5">
